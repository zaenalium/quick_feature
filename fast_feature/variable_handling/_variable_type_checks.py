...
def _is_categorical_and_is_not_datetime(column: pl.Series) -> bool:
...
def _is_categories_num(column: pl.Series) -> bool:
    return is_numeric(column.dtype.categories)


def _is_convertible_to_dt(column: pl.Series) -> bool:
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        return is_datetime(pd.to_datetime(column, errors="ignore", utc=True))


def _is_convertible_to_num(column: pl.Series) -> bool:
    try:
...
def _is_categorical_and_is_datetime(column: pl.Series) -> bool:
    # check for datetime only if object cannot be cast as numeric because
    # if it could pd.to_datetime would convert it to datetime regardless
    if is_object(column):
