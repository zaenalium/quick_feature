...
from fast_feature.variable_handling._variable_type_checks import (
    _is_categorical_and_is_datetime,
    _is_categorical_and_is_not_datetime,
)
from fast_feature.variable_handling.dtypes import DATETIME_TYPES


def find_numerical_variables(X: pl.DataFrame) -> List[Union[str, int]]:
    """
    ...
    --------
    >>> import polars as pl
    >>> from fast_feature.variable_handling import find_numerical_variables
    >>> X = pl.DataFrame({
    >>>     "var_num": [1, 2, 3],
    >>>     "var_cat": ["A", "B", "C"],
    ...
    ['var_num']
    """
    variables = X.select(cs.numeric()).columns #list(X.select_dtypes(include="number").columns)
    if len(variables) == 0:
        raise TypeError(
...
def find_categorical_variables(X: pl.DataFrame) -> List[Union[str, int]]:
    ...
    --------
    >>> import polars as pl
    >>> from fast_feature.variable_handling import find_categorical_variables
    >>> X = pl.DataFrame({
    >>>     "var_num": [1, 2, 3],
    >>>     "var_cat": ["A", "B", "C"],
    >>>     "var_date": pd.date_range("2020-02-24", periods=3, freq="T")
    ...
    ['var_cat']
    """
    variables = X.select(cs.by_dtype(pl.String, pl.Categorical)).columns
    if len(variables) == 0:
        raise TypeError(
            "No categorical variables found in this dataframe. Please check "
            "variable format with polars dtypes."
        )
    return variables


def find_datetime_variables(X: pl.DataFrame) -> List[Union[str, int]]:
    """
    ...
    --------
    >>> import polars as pl
    >>> from fast_feature.variable_handling import find_datetime_variables
    >>> X = pl.DataFrame({
    >>>     "var_num": [1, 2, 3],
    >>>     "var_cat": ["A", "B", "C"],
    >>>     "var_date": pd.date_range("2020-02-24", periods=3, freq="T")
...
def find_all_variables(
    ...
    --------
    >>> import polars as pl
    >>> from fast_feature.variable_handling import find_all_variables
    >>> X = pl.DataFrame({
    >>>     "var_num": [1, 2, 3],
    >>>     "var_cat": ["A", "B", "C"],
    >>>     "var_date": pd.date_range("2020-02-24", periods=3, freq="T")
    ...
    if exclude_datetime is True:
        variables = X.select(cs.all() - cs.temporal()).columns
        # variables = [
        #     var
        #     for var in variables
        #     if X[var].is_numeric or not _is_categorical_and_is_datetime(X[var])
        # ]
    else:
        variables = X.columns
    return variables
