...
class DecisionTreeDiscretiser(BaseNumericalTransformer):
    ...
    >>> import numpy as np
    >>> import pandas as pd
    >>> from fast_feature.discretisation import DecisionTreeDiscretiser
    >>> np.random.seed(42)
    >>> X = pl.DataFrame(dict(x= np.random.randint(1,100, 100)))
    >>> y_reg = pl.Series(np.random.randn(100))
    ...
    >>> y_clf = pl.Series(np.random.randint(0,2,100))
    ...
    def fit(self, X: pl.DataFrame, y: pl.Series):  # type: ignore
        """
    ...
    def transform(self, X: pl.DataFrame) -> pl.DataFrame:
        ...
        if self.bin_output == "prediction":
            for feature in self.variables_:
                if self.regression:
                    ...
                    if self.precision is None:
                        X = X.with_columns(pl.lit(preds).alias(feature))
                    else:
                        X = X.with_columns(pl.lit(np.round(preds, self.precision)).alias(feature))
                else:
            ...
            for feature in self.variables_:
                ...
                X = X.with_columns(pl.lit(cutt).cast(pl.String).alias(feature))
            ...
            for feature in self.variables_:
                ...
                X = X.with_columns(pl.lit(cutt).cast(pl.Int64).alias(feature))
