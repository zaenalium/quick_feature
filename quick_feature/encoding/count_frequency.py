...
class CountFrequencyEncoder(CategoricalInitMixinNA, CategoricalMethodsMixin):
    ...
    >>> X = pd.DataFrame(dict(x1 = [1,2,3,4], x2 = ["c", "a", "b", "c"])) # or X = pl.DataFrame(dict(x1 = [1,2,3,4], x2 = ["c", "a", "b", "c"])) 
    ...
    def fit(self, X: pl.DataFrame, y: Optional[pl.Series] = None):
        ...
        y: pandas or polars Series, default = None
        ...
        for var in variables_:
            if self.encoding_method == "count":
                self.encoder_dict_[var] = X[var].value_counts()\
                                            .to_pandas().set_index(var)['count'].to_dict()

            elif self.encoding_method == "frequency":
                self.encoder_dict_[var] = X[var].value_counts(normalize=True)\
                                            .to_pandas().set_index(var)['proportion'].to_dict()
            else:
                raise ValueError(
                    "Unrecognized value for encoding_method. It should be 'count' or "
                    f"'frequency'. Got {self.encoding_method} instead."
                )
        print(self.encoder_dict_[var])
