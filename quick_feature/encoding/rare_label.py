...
class RareLabelEncoder(CategoricalInitMixinNA, CategoricalMethodsMixin):
    ...
    **Note**
    ...
    >>> X = pd.DataFrame(dict(x1 = [1,2,3,4,5,6], x2 = ["b", "b", "b", "b", "b", "a"])) # or  X = pl.DataFrame(dict(x1 = [1,2,3,4,5,6], x2 = ["b", "b", "b", "b", "b", "a"]))
    ...
    def fit(self, X: pl.DataFrame, y: Optional[pl.Series] = None):
        ...
        for var in variables_:
            X = X.with_columns(pl.col(var).cast(pl.String).alias(var))
            if len(X[var].unique()) > self.n_categories:

                # if the variable has more than the indicated number of categories
                # the encoder will learn the most frequent categories
                t = X[var].value_counts(normalize=True).sort('proportion', descending = True)

                # non-rare labels:
                #freq_idx = t[t >= self.tol].index
                freq_idx = t.filter(pl.col('proportion') >= self.tol)[var].to_list()
                ...
                self.encoder_dict_[var] = list(X[var].unique(maintain_order = True))
    ...
    def transform(self, X: pl.DataFrame) -> pl.DataFrame:
        ...
        for feature in self.variables_:
            ...
            X= X.with_columns(pl.col(feature).cast(pl.String).alias(feature)) #("O")

            X = X.with_columns(
                    pl.when(pl.col(feature).is_in(self.encoder_dict_[feature] + with_nan).not_()
                            ).then(pl.lit(self.replace_with)).otherwise(pl.col(feature)).alias(feature))
