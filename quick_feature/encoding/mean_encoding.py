...
from quick_feature.dataframe_checks import check_X_y
...
class MeanEncoder(CategoricalInitMixinNA, CategoricalMethodsMixin):
    ...
    >>> X = pd.DataFrame(dict(x1 = [1,2,3,4,5], x2 = ["c", "c", "c", "b", "a"])) # or X = pl.DataFrame(dict(x1 = [1,2,3,4,5], x2 = ["c", "c", "c", "b", "a"]))
    ...
    def fit(self, X: pl.DataFrame, y: pl.Series):
        ...
        for var in variables_:
            if self.smoothing == "auto":
                damping = X.with_columns(__y__ = y).group_by(var).agg(pl.col('__y__').var(ddof=0)/y_var)
            else:
                damping = pl.DataFrame({var : X[var].unique(maintain_order=True), '__y__' : self.smoothing})
            counts = X[var].value_counts()
            counts = counts.join(damping, on = var).with_columns(
                            _lambda = pl.col('count')/(pl.col('__y__') + pl.col('count')))
            mean_x = X.with_columns(y = y).group_by(var).agg(pl.col('y').mean().alias('__mean__'))
            counts = counts.join(mean_x, on = var)
            counts = counts.with_columns(enc = pl.col('_lambda') * pl.col('__mean__')
                            + (1.0 - pl.col('_lambda')) * y_prior)
            counts = counts.select(var,'enc')
            self.encoder_dict_[var] = counts.to_pandas().set_index(var)['enc'].to_dict()
