...
from quick_feature.dataframe_checks import _check_contains_na, check_X
...
class OneHotEncoder(CategoricalInitMixin, CategoricalMethodsMixin):
    ...
    **Also Note**
    ...
    >>> X = pd.DataFrame(dict(x1 = [1,2,3,4], x2 = ["a", "a", "b", "c"])) # or X = pl.DataFrame(dict(x1 = [1,2,3,4], x2 = ["a", "a", "b", "c"]))
    >>> ohe = OneHotEncoder()
    >>> ohe.fit(X)
    >>> ohe.transform(X)
       x1  x2_a  x2_b  x2_c
    ...
    def fit(self, X: pl.DataFrame, y: Optional[pl.Series] = None):
        ...
        for var in variables_:
            ...
            if self.top_categories:
                self.encoder_dict_[var] = [
                    ...
                    .head(self.top_categories)[var].to_list()
                ]

            else:
                category_ls = list(X[var].unique(maintain_order = True))
        ...
        self.variables_binary_ = [var for var in variables_ if X[var].n_unique() == 2]

        # automatically encode binary variables as 1 dummy
        if self.drop_last_binary:
            for var in self.variables_binary_:
                category = X[var].unique(maintain_order = True)[0]
    ...
    def transform(self, X: pl.DataFrame) -> pl.DataFrame:
        ...
        Returns
        -------
        X_new: polars or pandas dataframe.
        ...
        for feature in self.variables_:
            for category in self.encoder_dict_[feature]:
                ...
                X = pl.concat([X, dummy_df] , how="horizontal") 

        # drop the original non-encoded variables.
        X = X.drop(self.variables_)
