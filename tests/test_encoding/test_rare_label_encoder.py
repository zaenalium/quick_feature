    ...
    tt = pl.DataFrame(
        {
            "var_A": ["A", None, "Rare", "G"],
            "var_B": ["A", None, "Rare", "G"],
            "var_C": [3.0, None, "Rare", "Rare"],
        }, strict=False
    )

    X = encoder.transform(t)
    assert_frame_equal(X, tt, check_dtype=False)
...
def test_variables_cast_as_category_with_na_in_transform(df_enc_big):
    ...
    assert_frame_equal(encoder.transform(t), tt)
