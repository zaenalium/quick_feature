...
def test_variables_cast_as_category(df_enc_category_dtypes):
    ...
    assert_frame_equal(X, pl.from_pandas(transf_df[["var_A", "var_B"]]), check_dtype=False)
...
def test_encoding_new_categories(df_enc):
    df_unseen = pl.DataFrame({"var_A": ["D"], "var_B": ["D"]})
    encoder = MeanEncoder(unseen="encode")
    encoder.fit(df_enc[["var_A", "var_B"]], df_enc["target"])
    df_transformed = encoder.transform(df_unseen)
    assert (df_transformed.to_pandas() == df_enc["target"].mean()).all(axis=None)
